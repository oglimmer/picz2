...
user's own S3-compatible bucket — the form is written for a self-hosted MinIO and names no commercial
service (App Store in-app purchase rules, D88), but every field is free, so any S3-compatible endpoint works —
