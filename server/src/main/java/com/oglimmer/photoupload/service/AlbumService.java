...
public class AlbumService {
  ...
  public AlbumInfo duplicateAlbum(Long sourceAlbumId) {
    ...
    // Ensure the `all` system tag exists for this user
    Tag allTag = ensureAllTagExists(currentUser);
    ...
    for (FileMetadata source : sourceFiles) {
      ...
      // Every new asset carries the `all` tag (D68), and a duplicate is a new asset.
   ...
   */
  private Tag ensureAllTagExists(User user) {
    Long allTagId = systemTagProvisioner.ensureTag(user, FileStorageService.ALL_TAG);
    ...
    return tagRepository.getReferenceById(allTagId);
  }
