 ...
 *
 * <p>Every successful response is HTTP 200 with a {@link TusHookResponse} JSON body; tusd decodes
 * the body and uses its {@code HTTPResponse.StatusCode} to decide what to surface to the client.
 * Returning non-2xx (or an empty body) would make tusd log "failed to parse hook response" and
 * propagate 500 to the client — see {@link TusHookResponse} for the schema.
 *
 * <p>Path-secret mismatch is the one case where we *don't* return a hook-shaped JSON: a probe with
 * the wrong secret gets the same 404 an unmapped URL would yield, so the path's existence isn't
 * observable. tusd itself never calls with a wrong secret because the secret is a config value, not
 * user input.
 */
...
public class TusHookController {
  ...
  /**
   * Constant-time equality. Standard {@link String#equals} short-circuits on the first differing
   * byte, leaking position information through timing — irrelevant for cluster-internal use today,
   * but cheap insurance against future deploy mistakes that put the path on a public Ingress.
   */
