...
/**
 * Reflects the MinIO circuit-breaker state into Spring Boot's readiness group. When the breaker is
 * OPEN, this indicator goes DOWN — K8s removes the api pod from the Service for the duration, so
 * traffic stops reaching it until the breaker auto-transitions back to HALF_OPEN/CLOSED.
 *
...
public class MinioHealthIndicator implements HealthIndicator {
  ...
  public Health health() {
    ...
    return switch (state) {
      ...
      case HALF_OPEN -> Health.unknown().withDetail("breaker", "HALF_OPEN").build();
      case OPEN, FORCED_OPEN -> Health.down().withDetail("breaker", state.name()).build();
