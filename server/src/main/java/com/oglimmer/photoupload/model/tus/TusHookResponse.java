 ...
 * shape; the {@code HTTPResponse.StatusCode} inside is what tusd actually surfaces to the client.
 * Returning a non-2xx HTTP status from the controller (or an empty body) makes tusd log "failed to
 * parse hook response" and propagate 500 to the client — that was the R1-deploy bug.
 *
 * <p>Field names are PascalCase per tusd's Go convention, mapped via {@link JsonProperty}. Null
 * fields are omitted ({@link JsonInclude.Include#NON_NULL}) so {@link #allow()} renders as {@code
 * {"RejectUpload":false}} rather than carrying a useless empty {@code HTTPResponse}.
 */
...
public record TusHookResponse(
  ...
  /**
   * Reject with a specific HTTP status. tusd surfaces {@code statusCode} + {@code body} + {@code
   * header} verbatim to the client. Used for 401/409/503.
   */
